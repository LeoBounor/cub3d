...
/*   Updated: 2023/06/25 18:46:19 by jcollon          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	check_border(t_game *game)
{
	...
	{
		if (game->game_map[i][0] != 1 && game->game_map[i][0] != 2)
			return (1);
		else if (game->game_map[i][game->game_tab_width - 1] != 1 \
			&& game->game_map[i][game->game_tab_width - 1] != 2)
			return (1);
		i++;
	}
	i = 0;
	...
	{
		if (game->game_map[0][i] != 1 && game->game_map[0][i] != 2)
			return (1);
		else if (game->game_map[game->game_tab_height - 1][i] != 1 \
			&& game->game_map[game->game_tab_height - 1][i] != 2)
			return (1);
		i++;
	}
	return (0);
...
{
	if (game->game_map[i][j] == 'N' - '0' || game->game_map[i][j] == 'S' - '0' \
	|| game->game_map[i][j] == 'E' - '0' || game->game_map[i][j] == 'W' - '0')
	{
		game->y = i * 64 + 32;
		game->x = j * 64 + 32;
		if (game->game_map[i][j] == 'N' - '0')
			game->player_angle = (3 * M_PI) / 2;
		else if (game->game_map[i][j] == 'S' - '0')
			game->player_angle = M_PI / 2;
		else if (game->game_map[i][j] == 'E' - '0')
			game->player_angle = 0;
		else if (game->game_map[i][j] == 'W' - '0')
...
{
	if ((game->game_map[i + 1][j] == 2 || game->game_map[i - 1][j] == 2 || \
	game->game_map[i][j + 1] == 2 || game->game_map[i][j - 1] == 2) && \
	free_map(game->game_map, game->game_tab_height - 1) && clear_textures(game))
...
void	check_map(t_game *game, int i, int j)
{
	...
	while (i != game->game_tab_height - 1)
	{
		...
		while (j != game->game_tab_width - 1)
		{
			...
			if (game->game_map[i][j] == 0 || game->game_map[i][j] == 'N' || \
				game->game_map[i][j] == 'S' || game->game_map[i][j] == 'E' || \
				game->game_map[i][j] == 'W')
	...
	3) && free_map(game->game_map, game->game_tab_height - 1) \
	&& clear_textures(game))
		ft_err_map("Map error\n", game->fd_str, game);
